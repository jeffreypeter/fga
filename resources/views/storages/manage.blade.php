			...
			<div class="col-12">
				<div class="card">
					<div class="card-header">
						<strong>Manage Storages</strong>
					</div>
					<div class="card-body">
						<a href="{{ URL::to('storages/create') }}" class="btn btn-primary">Add Storage</a>
						<div class="shell">
							<table id="manage-storages" class="display responsive" width="100%">
								<thead>
								<tr>
									{{--<th>Id</th>--}}
									<th>Name</th>
								...
								@foreach($storages as $key => $value)
									<tr>
										<td><a href="{{ URL::to('storages/' . $value->id. '/edit') }}">{{ $value->name }}</a></td>
										<td>{{ $value->address }}</td>
										<td>
											<div class="btn-group actions" role="group" aria-label="actions">
												<button onclick="window.location.href='{{ URL::to('storages/' . $value->id. '/edit') }}'" class="btn btn-info btn-secondary"><i class="fa fa-edit" aria-hidden="true"></i></button>
												{{ Form::open(array('url' => 'storages/' . $value->id, 'class'=>'pull-left')) }}
...
@section('myscript')
	...
	<script src="{{ asset('js/views/storages/manage.js') }}"></script>
@endsection